...
export const QUESTS = [
  ...
  /* ---------------------------------------------------------------- 5 -- */
  {
    ...
    practice: {
      ...
      hint: 'corp.modules — то, что стоит на корабле. Модуль попадает туда со склада: '
        + 'купили на верфи, установили в разделе «Склад» — и он в сборке.',
      ...
      validate: (value, context) => {
        ...
        if ((context.corp.modules?.length ?? 0) === 0) {
          return 'На корабле нет модулей: купите их на верфи и установите в разделе «Склад» — '
            + 'лежащие на складе в сборку не идут';
        }
        if (value.mass === 0) return 'Масса нулевая — передайте модули корабля: corp.modules';
        return true;
      },
      commit: (value, api) => {
  ...
  /* -------------------------------------------------- 10 -- этап 2 из 3 -- */
  {
    ...
    extends: 'plan',
    title: 'План с грузом',
    topic: 'Доработка функции: новое слагаемое в расчёте',
    difficulty: 3,
    reward: { credits: 26000, xp: 160 },
    story:
      'Первый же рейс показал: расчёт врёт. Корабль возвращается гружёным, ' +
      'а топливо считалось по пустому. Диспетчер требует переделать план.',
    signature: 'planFlight(ship, distance) → { distance, fuel, hours }',
    brief:
      'Доработайте свою функцию planFlight. Сигнатура прежняя, но у корабля '
      + 'появилось новое поле.\n'
      + '• ship.cargo — груз на борту в тоннах. Его может не быть вовсе.\n'
      + 'Топливо считается по полной массе: собственная масса корабля плюс груз.\n'
      + 'Всё остальное работает как раньше: округление вверх, часы от расстояния, '
      + 'поле distance возвращается как есть.\n'
      + 'Корабль без груза должен дать ровно тот же ответ, что и до доработки — '
      + 'старые проверки никто не отменял.',
    changes: {
      works: 'planFlight уже считает топливо по массе корабля и время по расстоянию.',
      changed: 'У корабля появилось поле cargo. Оно необязательное: у старых вызовов его нет.',
      todo: 'Прибавить груз к массе при расчёте топлива, ничего больше не меняя.',
    },
    lesson: [
      {
        title: 'Необязательное поле и запасное значение',
        text:
          'Если поля нет, обращение к нему даёт undefined, а сложение с ним портит всё '
          + 'выражение. Оператор ?? подставляет запасное значение только для undefined и null, '
          + 'а ноль и пустую строку оставляет как есть.',
        code:
          'const ship = { mass: 430 };\n'
          + '\n'
          + 'ship.cargo            // undefined\n'
          + '430 + ship.cargo      // NaN — расчёт испорчен\n'
          + '430 + (ship.cargo ?? 0)   // 430 — порядок',
      },
      {
        title: 'Доработка не значит переписать заново',
        text:
          'В редакторе открыт ваш собственный код с прошлого этапа. Менять нужно ровно '
          + 'одно место — то, где считается топливо. Остальные строки трогать незачем: '
          + 'они уже проверены и работают.',
        code:
          '// было\n'
          + 'const fuel = Math.ceil(ship.mass * distance / 1000);\n'
          + '\n'
          + '// стало: та же формула, но масса полная\n'
          + 'const total = ship.mass + (ship.cargo ?? 0);\n'
          + 'const fuel = Math.ceil(total * distance / 1000);',
      },
      {
        title: 'Старое поведение — это тоже требование',
        text:
          'Проверки прошлого этапа запускаются снова. Это не придирка: на вашу функцию '
          + 'уже опираются разделы «Маршруты» и «Экспедиция». Сломав расчёт для пустого '
          + 'корабля, вы сломаете их обоих.',
      },
    ],
    theory: [
      'ship.cargo ?? 0 — груз, если он указан, и ноль, если поля нет.',
      'Полная масса складывается до умножения на расстояние.',
      'Часы по-прежнему зависят только от расстояния: груз их не меняет.',
      'Проверки прошлого этапа прогоняются заново — старый ответ должен сохраниться.',
    ],
    fn: 'planFlight',
    starter:
      'function planFlight(ship, distance) {\n'
      ...
      + '}\n',
    hints: [
      'Менять нужно одну строку — ту, где считается топливо.',
      'Груза может не быть: подставьте ноль вместо отсутствующего поля.',
      'Посчитайте полную массу в отдельную переменную, а формулу топлива оставьте прежней.',
    ],
    solution:
      'function planFlight(ship, distance) {\n'
      + '  const loaded = ship.mass + (ship.cargo ?? 0);\n'
      + '\n'
      + '  const fuel = Math.ceil(loaded * distance / 1000);\n'
      + '  const hours = Math.ceil(distance / 12);\n'
      + '\n'
      + '  return { distance, fuel, hours };\n'
      + '}\n',
    practice: {
      title: 'Пересчитайте план с грузом',
      refresh: true,
      hint:
        'corp.ship теперь знает, сколько руды поместится в трюм: поле cargo. '
        + 'Пересчитайте план — в базу ляжет новая версия.',
      example: 'db.insert("plans", planFlight(corp.ship, 240))',
      validate: value => {
        if (!value || typeof value !== 'object') return 'Команда должна вернуть сохранённый план полёта';
        if (typeof value.fuel !== 'number') return 'В плане нет числового поля fuel — передайте результат planFlight';
        if (value.fuel <= 0) return 'Расход нулевой: проверьте, что на корабле стоят модули';
        if (typeof value.id !== 'number') return 'План не попал в базу: оберните результат в db.insert("plans", …)';
        return true;
      },
      commit: value => `План пересчитан с грузом: ${value.fuel} т топлива на ${value.hours} ч пути`,
    },
    inheritTests: true,
    tests: [
      {
        name: 'Гружёный корабль ест больше',
        args: [{ mass: 430, cargo: 150 }, 240],
        expected: { distance: 240, fuel: 140, hours: 20 },
      },
      {
        name: 'Пустой трюм ничего не меняет',
        args: [{ mass: 430, cargo: 0 }, 240],
        expected: { distance: 240, fuel: 104, hours: 20 },
      },
      {
        name: 'Поля cargo может не быть вовсе',
        args: [{ mass: 430 }, 240],
        expected: { distance: 240, fuel: 104, hours: 20 },
      },
      {
        name: 'Груз не влияет на время в пути',
        expr: 'return planFlight({ mass: 430, cargo: 600 }, 240).hours;',
        expected: 20,
      },
    ],
  },
  ...
  /* -------------------------------------------------- 11 -- этап 3 из 3 -- */
  {
    ...
    order: 11,
    extends: 'plan-cargo',
    title: 'Аварийный резерв',
    topic: 'Доработка функции: новое поле в результате',
    difficulty: 3,
    reward: { credits: 28000, xp: 170 },
    story:
      'Совет Колоний ввёл правило: в баке должен оставаться запас на случай ' +
      'сноса с курса. Летать «впритык» больше не разрешают.',
    signature: 'planFlight(ship, distance) → { distance, fuel, hours, reserve, total }',
    brief:
      'Снова дорабатываем planFlight. Прежние поля остаются, добавляются два новых.\n'
      + '• reserve — аварийный запас: десятая часть расхода, округлённая ВВЕРХ;\n'
      + '• total — сколько топлива нужно залить всего: расход плюс резерв.\n'
      + 'Поля distance, fuel и hours считаются в точности как раньше: fuel — это '
      + 'по-прежнему расход на сам рейс, без резерва.\n'
      + 'Заправка и экспедиция начнут спрашивать total, поэтому от правильности этого '
      + 'поля зависит, состоится ли рейс.',
    changes: {
      works: 'planFlight считает топливо по массе с грузом и время по расстоянию.',
      changed: 'В результате появляются два новых поля. Старые остаются нетронутыми.',
      todo: 'Посчитать резерв в десятую часть расхода и общую потребность.',
    },
    lesson: [
      {
        title: 'Новое поле не ломает старые',
        text:
          'Добавить поле в возвращаемый объект безопасно: тот, кто про него не знает, '
          + 'просто его не читает. Поэтому старые проверки продолжают проходить, а разделы '
          + 'начинают пользоваться новым полем по мере готовности.',
        code:
          '// было\n'
          + 'return { distance, fuel, hours };\n'
          + '\n'
          + '// стало: прежние поля на месте\n'
          + 'return { distance, fuel, hours, reserve, total };',
      },
      {
        title: 'Проценты через округление вверх',
        text:
          'Десятая часть — это деление на десять. Но топливо целое, а запас, округлённый '
          + 'вниз, перестаёт быть запасом. Поэтому снова Math.ceil.',
        code:
          'Math.ceil(104 / 10)   // 11\n'
          + 'Math.ceil(100 / 10)   // 10\n'
          + 'Math.ceil(3 / 10)     // 1 — даже у крошечного рейса запас есть',
      },
      {
        title: 'Считайте по порядку, шаг за шагом',
        text:
          'Резерв зависит от расхода, а общая потребность — от обоих. Если писать всё '
          + 'одним выражением, ошибку потом не найти. Отдельные переменные читаются лучше '
          + 'и отлаживаются проще.',
        code:
          'const fuel = Math.ceil(loaded * distance / 1000);\n'
          + 'const reserve = Math.ceil(fuel / 10);\n'
          + 'const total = fuel + reserve;',
      },
    ],
    theory: [
      'reserve — это Math.ceil(fuel / 10).',
      'total — это fuel + reserve, отдельное поле.',
      'Поле fuel остаётся расходом на рейс: резерв в него не входит.',
      'Проверки обоих прошлых этапов запускаются заново.',
    ],
    fn: 'planFlight',
    starter:
      'function planFlight(ship, distance) {\n'
      ...
      + '}\n',
    hints: [
      'Прежние строки не трогайте: добавляются два расчёта и два поля в ответе.',
      'Резерв считается от расхода, а не от массы и не от расстояния.',
      'Общая потребность — это сумма расхода и резерва; её и будет спрашивать заправка.',
    ],
    solution:
      'function planFlight(ship, distance) {\n'
      + '  const loaded = ship.mass + (ship.cargo ?? 0);\n'
      ...
      + '  const reserve = Math.ceil(fuel / 10);\n'
      + '\n'
      + '  return { distance, fuel, hours, reserve, total: fuel + reserve };\n'
      + '}\n',
    practice: {
      title: 'Утвердите план с резервом',
      refresh: true,
      hint:
        'Пересчитайте план заново: теперь в нём есть общая потребность в топливе. '
        + 'Именно её спрашивают «Маршруты» и «Экспедиция».',
      example: 'db.insert("plans", planFlight(corp.ship, 240))',
      validate: value => {
        if (!value || typeof value !== 'object') return 'Команда должна вернуть сохранённый план полёта';
        if (typeof value.reserve !== 'number') return 'В плане нет числового поля reserve — добавьте аварийный запас';
        if (typeof value.total !== 'number') return 'В плане нет числового поля total — это расход плюс резерв';
        if (value.total <= value.fuel) return 'Общая потребность должна быть больше расхода: резерв в неё не вошёл';
        if (typeof value.id !== 'number') return 'План не попал в базу: оберните результат в db.insert("plans", …)';
        return true;
      },
      commit: value =>
        `План утверждён: ${value.fuel} т на рейс и ${value.reserve} т резерва, залить ${value.total} т`,
    },
    inheritTests: true,
    tests: [
      {
        name: 'Резерв — десятая часть расхода',
        args: [{ mass: 430 }, 240],
        expected: { distance: 240, fuel: 104, hours: 20, reserve: 11, total: 115 },
      },
      {
        name: 'Гружёный рейс требует больше и резерва',
        args: [{ mass: 430, cargo: 150 }, 240],
        expected: { distance: 240, fuel: 140, hours: 20, reserve: 14, total: 154 },
      },
      {
        name: 'Резерв округляется вверх',
        expr: 'return planFlight({ mass: 100 }, 100).reserve;',
        expected: 1,
      },
      {
        name: 'Расход остался расходом, без резерва',
        expr: 'return planFlight({ mass: 430 }, 240).fuel;',
        expected: 104,
      },
    ],
  },
  ...
  /* ------------------------------------------------ 13 -- диагностика -- */
  {
    id: 'ledger',
    order: 13,
    title: 'Ревизия топлива',
    topic: 'Поиск ошибки в чужом коде, журнал операций',
    difficulty: 4,
    reward: { credits: 32000, xp: 200 },
    unlocks: { view: 'audit', label: 'Ревизия' },
    story:
      'После рейса бак показал меньше, чем должен был. Диспетчер прислал ' +
      'журнал операций и готовую функцию сверки — с ней что-то не так. ' +
      'Найти ошибку придётся вам.',
    signature: 'fuelBalance(entries) → { in, out, left }',
    brief:
      'В редакторе уже лежит функция fuelBalance. Она считает движение топлива '
      + 'по журналу операций и возвращает объект:\n'
      + '• in — сколько всего залито;\n'
      + '• out — сколько всего израсходовано;\n'
      + '• left — остаток: залито минус израсходовано.\n'
      + 'Запись журнала: { kind, amount }, где kind — строка "fill" для заправки '
      + 'и "burn" для расхода, а amount — всегда положительное число тонн.\n'
      + 'Функция написана, но считает неверно. Запустите проверки, прочитайте, '
      + 'что именно разошлось, и почините. Переписывать всё заново не нужно — '
      + 'ошибка в одном месте.',
    changes: {
      works: 'Функция уже обходит журнал и складывает суммы по видам операций.',
      changed: 'Ничего не менялось — код изначально с ошибкой.',
      todo: 'Найти место, где расход учитывается неправильно, и исправить его.',
    },
    lesson: [
      {
        title: 'Журнал операций — это список событий',
        text:
          'Каждая запись говорит, что произошло и на сколько. Остаток не хранится: '
          + 'его получают, пройдя журнал от начала до конца. Поэтому одна лишняя '
          + 'строка в цикле уводит весь баланс.',
        code:
          'const entries = [\n'
          + '  { kind: "fill", amount: 200 },\n'
          + '  { kind: "burn", amount: 104 },\n'
          + '];\n'
          + '\n'
          + '// залито 200, израсходовано 104, остаток 96',
      },
      {
        title: 'Читайте расхождение, а не код',
        text:
          'Отчёт о проверке показывает, что ждали и что получили. Если расход вышел '
          + 'ровно вдвое больше — значит, он прибавляется дважды. Если остаток не сходится '
          + 'на величину одной записи — значит, запись посчитана не в ту сторону. '
          + 'Ошибку быстрее найти по разнице чисел, чем перечитывая строки.',
      },
      {
        title: 'Двойное списание — классика',
        text:
          'Частая ошибка: величину вычитают из остатка и при этом прибавляют к расходу, '
          + 'а потом остаток считают ещё раз из этих сумм. Одно и то же действие выполняется '
          + 'дважды. Решается тем, что остаток считают один раз — в конце, из итоговых сумм.',
        code:
          '// опасно: остаток меняется и внутри цикла, и после него\n'
          + 'left -= entry.amount;\n'
          + '// …\n'
          + 'left = filled - burned;',
      },
    ...
    theory: [
      'Сумму по виду операции копят в отдельной переменной.',
      'Остаток считают один раз, в конце: залито минус израсходовано.',
      'Отчёт о проверке подсказывает, в какую сторону разошлись числа.',
      'Неизвестный вид операции просто пропускают.',
    ],
    fn: 'fuelBalance',
    starter:
      'function fuelBalance(entries) {\n'
      + '  let filled = 0;\n'
      + '  let burned = 0;\n'
      + '\n'
      ...
      + '    }\n'
      + '\n'
      + '    // где-то здесь ошибка: расход учитывается не так, как надо\n'
      + '    burned += entry.amount;\n'
      + '  }\n'
      + '\n'
      + '  return { in: filled, out: burned, left: filled - burned };\n'
      + '}\n',
    hints: [
      'Посмотрите, при каких записях увеличивается burned. Точно ли только при расходе?',
      'Строка с burned стоит вне условия, поэтому выполняется для каждой записи журнала.',
      'Расход должен прибавляться только когда kind равен "burn" — как залив прибавляется только при "fill".',
    ],
    solution:
      'function fuelBalance(entries) {\n'
      + '  let filled = 0;\n'
      ...
      + '}\n',
    practice: {
      title: 'Сведите баланс топлива',
      hint:
        'Журнал операций корпорации лежит в corp.fuelLog. Сведите по нему баланс '
        + 'и запишите ревизию в базу.',
      example: 'db.insert("audits", fuelBalance(corp.fuelLog))',
      validate: (value, context) => {
        if (!value || typeof value !== 'object') return 'Команда должна вернуть сохранённую ревизию';
      ...
      commit: value => `Ревизия сошлась: залито ${value.in} т, израсходовано ${value.out} т, остаток ${value.left} т`,
    },
    tests: [
      {
        name: 'Обычный журнал',
        args: [[{ kind: 'fill', amount: 200 }, { kind: 'burn', amount: 104 }]],
        expected: { in: 200, out: 104, left: 96 },
      },
      {
        name: 'Несколько заправок и рейсов',
        args: [[
          { kind: 'fill', amount: 200 },
          { kind: 'burn', amount: 104 },
          { kind: 'fill', amount: 50 },
        ...
        expected: { in: 250, out: 134, left: 116 },
      },
      { name: 'Пустой журнал', args: [[]], expected: { in: 0, out: 0, left: 0 } },
      {
        name: 'Только заправки',
        args: [[{ kind: 'fill', amount: 100 }]],
        expected: { in: 100, out: 0, left: 100 },
      },
      {
        name: 'Неизвестные операции пропускаются',
        args: [[{ kind: 'fill', amount: 100 }, { kind: 'sale', amount: 999 }]],
        expected: { in: 100, out: 0, left: 100 },
      },
    ],
  },
  ...
  /* --------------------------------------------------------------- 12 -- */
  {
    ...
    practice: {
      ...
      hint:
        'Модули корабля лежат в corp.modules. Посчитайте по ним сводку и ' +
...
/**
 * Все проверки задания: собственные плюс унаследованные от прошлых этапов.
 *
 * Доработка не должна ломать то, что уже работает: на функцию опираются
 * разделы интерфейса и другие функции игрока. Поэтому прежние проверки
 * прогоняются заново и помечаются как проверки прежнего поведения.
 */
export function testsOf(quest) {
  if (!quest) return [];
  if (!quest.inheritTests || !quest.extends) return quest.tests;

  const parent = questById(quest.extends);
  const inherited = testsOf(parent).map(test => ({
    ...test,
    // Пометка не дублируется, когда этапов больше двух
    name: test.inherited ? test.name : `${test.name} · прежнее поведение`,
    inherited: true,
    // Доработка вправе добавить в ответ новые поля: прежняя проверка
    // следит за тем, что старые значения не изменились, а не за тем,
    // что объект остался прежним до последнего ключа
    subset: true,
  }));

  return [...inherited, ...quest.tests];
}

/** Этапы развития одной функции по порядку цепочки. */
export function stageChain(fnName) {
  return [...QUESTS].sort((a, b) => a.order - b.order).filter(quest => quest.fn === fnName);
}
